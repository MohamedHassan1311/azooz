...
class PayWithVisaScreen extends StatefulWidget {
  const PayWithVisaScreen({
    Key? key,
  ...
  @override
  State<PayWithVisaScreen> createState() => _PayWithVisaScreenState();
}

class _PayWithVisaScreenState extends State<PayWithVisaScreen> {
  @override
  void initState() {
    super.initState();
    print("I am comming from Visa screen");
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        VisaLogoWidget(
          onTapVisa: () => context.read<PaymentProvider>().setPaymentMethod =
                ...
                CustomButton(
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      context
                          .read<PaymentProvider>()
                          .payWithVisa(widget.paymentCheckoutModel, payRequest)
                          .then((value) {
                        if (value == true) {
                  ...
                  text:
                      "مبلغ ${widget.paymentCheckoutModel.amount!.toPrecision(3)} ريال",
                ),
              ],
            ),
          );
...
class VisaLogoWidget extends StatelessWidget {
  void Function() onTapVisa;

  VisaLogoWidget({
    Key? key,
    required this.onTapVisa,
  }) : super(key: key);
